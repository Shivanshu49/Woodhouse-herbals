...
- **ReelsSection** ("Watch It. Love It."): section `bg-white`, same heading treatment as Best Seller (bold + italic Fraunces). Reel cards `aspect-[9/16] rounded-3xl` with play overlay + caption gradient; thumbnails live in `/public/reels` (Instagram CDN URLs expire — see `src/data/reels.ts`). Click opens the reel in an on-site embed modal; last slide is a `bg-brand-teal` follow CTA to @woodhouseherbals.
