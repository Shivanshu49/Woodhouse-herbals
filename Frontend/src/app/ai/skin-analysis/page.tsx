  ...
  return (
    <section className="container-wide py-10 sm:py-14">
      <div className="max-w-3xl mx-auto">
          ...
          {STEPS.map((label, i) => (
            <li
              ...
              className={cn(
                ...
                i <= step ? 'bg-forest-900 text-cream' : 'bg-cream-200 text-ink-muted',
              )}
            >
              <span
                className={cn(
                  'inline-flex h-5 w-5 items-center justify-center rounded-full text-[10px] font-semibold',
                  i < step ? 'bg-sage-200 text-forest-900' : i === step ? 'bg-cream text-forest-900' : 'bg-white/40',
                )}
              >
        ...
        <div className="mt-8 rounded-3xl bg-white border border-forest-900/5 shadow-soft p-6 sm:p-8">
          {step === 0 && (
            <>
              <h2 className="font-display text-2xl text-forest-900">What is your skin type?</h2>
          ...
          {step === 1 && (
            <>
              <h2 className="font-display text-2xl text-forest-900">What are your top concerns?</h2>
          ...
          {step === 2 && (
            <>
              <h2 className="font-display text-2xl text-forest-900">How much routine do you want?</h2>
                ...
                {ROUTINE.map((r) => (
                  <button
                    ...
                    className={cn(
                      ...
                      routine === r.value
                        ? 'border-forest-900 bg-sand-100/80'
                        : 'border-forest-900/10 hover:border-forest-900/30',
                    )}
                  >
                    <p className="font-medium text-forest-900">{r.label}</p>
                  </button>
                ))}
              </div>
            </>
          ...
          {step === 3 && (
            <>
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-display text-2xl text-forest-900">Your personalised ritual</h2>
              </div>
              ...
              <ul className="mt-6 grid sm:grid-cols-2 gap-3">
                {recommendations.map((p) => (
                  <li key={p.id} className="flex gap-3 rounded-2xl bg-sand-100/60 p-3 border border-forest-900/5">
                    <div className="relative h-20 w-20 shrink-0 overflow-hidden rounded-xl bg-white">
                      <Image src={p.thumbnail.url} alt={p.thumbnail.alt} fill sizes="80px" className="object-cover" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-forest-900 line-clamp-2">{p.name}</p>
                      <p className="text-xs text-ink-muted mt-0.5">{formatPrice(p.price)}</p>
                      <Link
                        href={`/shop/${p.slug}`}
                        className="mt-2 inline-flex items-center gap-1 text-xs font-medium text-forest-900 hover:text-forest-700"
                      >
          ...
          <div className="mt-8 flex items-center justify-between">
            ...
            {step < STEPS.length - 1 ? (
              <button
                ...
                className="inline-flex items-center gap-2 rounded-full bg-forest-900 text-cream px-6 py-2.5 text-sm font-medium hover:bg-forest-800 disabled:opacity-50"
              >
                Continue
              ...
              <Link
                href="/shop"
                className="inline-flex items-center gap-2 rounded-full bg-forest-900 text-cream px-6 py-2.5 text-sm font-medium hover:bg-forest-800"
              >
                Browse full shop
                <ArrowRight className="h-4 w-4" />
              </Link>
