...
- Payment `captured` but `amount !== Payment.amountMinor` → **anomaly hold**, not auto-fail: keep payment INITIATED, no order transition, persist a `payment_amount_mismatch` order-event once. The hold's **terminal is the persisted event itself** (CP5-REVISED — the earlier `RECONCILE_ANOMALY_MAX_OBSERVATIONS` counter was dead config: any value ≥1 behaved identically, and the row was re-fetched forever): the cron's `findMany` **excludes any order that already carries a `payment_amount_mismatch` event**, so a flagged payment is neither re-fetched nor able to pin a batch-cap slot — it waits for manual resolution (dashboard refund or forced settle). The abandonment branch still treats a held (captured-mismatched) row as *not abandonable*. ★REVIEW-FIX (Deliberate change from PhonePe, which cancelled + restocked on mismatch — dangerous when money actually moved. Near-impossible anyway: the amount is fixed on the server-created rzp Order.)
...
- **Payments sweep** (every 5 min): for each Payment `INITIATED` older than `RECONCILE_PAYMENT_MIN_AGE_MIN` (default 15) — age measured from the row's **creation** (initiate supersedes stale rows with fresh ones, §1.1, so creation age ≈ last initiate activity ★REVIEW-FIX):
  ...
  - a `captured` payment passing the **identical settle guard as the webhook/verify paths** (`status='captured' AND amount === amountMinor AND order_id === providerTxnId`) → settlePaymentSuccess ★REVIEW-FIX; a captured payment failing the amount guard → the §1.3 anomaly-hold flow (persist once → the order is then excluded from future sweeps, block abandonment);
...
| 7 | **PhonePe removal** | Delete `modules/phonepe/` (8 files) — this removes the `RF`-prefix refund-callback routing (`phonepe.service.ts:192-237`) that Phase 5's scope line named; it is **already unreachable since Phase 3 removed the PhonePe `express.raw` mount** (the callback controller 400s on a JSON-parsed body before `handleCallback` runs), so it is a dead second door, not a live one, in the Phase 5→7 window (CP4 review, low). Purge env.ts/`.env.example`; drop dead `bullmq` dep, add `@nestjs/schedule`; integrations key `phonepe`→`razorpay` (backend + Admin types + tab, one commit); audit-redact regex + new test; docs refresh: SECURITY.md (webhook model rewrite + fix the false redirect-validation claim at :30), architecture.md (fix the false BullMQ claim at :70 — the cron is now real), README, PRE-LAUNCH §2 rewrite (dashboard webhook registration on the API origin + failure-alert email + auto-disable warning + the **reconciliation dead-man alert** — CP5-REVISED: liveness = max over ALL THREE `reconcile:{payments,refunds,redrive}:last_completed_at` keys > 30 min, PAIRED with per-domain BACKLOG signals: oldest INITIATED payment age, oldest PENDING GATEWAY refund age, oldest processed=false razorpay WebhookEvent age (liveness alone is fresh even when every row of a sweep fails); §4.2 CF checklist if adopted; sandbox checklist) | gate ★REVIEW-FIX: `grep -ri phonepe` clean across **Backend/ + docs/** (excluding migration SQL + dated plan/spec records); Admin/Frontend strings are Phase 8/9 scope — the full repo-wide grep is the Phase 10 gate |
